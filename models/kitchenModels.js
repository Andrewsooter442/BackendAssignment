...
class kitchenModels {
    ...
    static async markComplete(order_id, item_id){
        ...
        try{
            ...
            const [result] = await connection.execute(`
                UPDATE order_items SET complete = true WHERE order_id=? AND item_id=?`,[order_id, item_id]
            );
            return result;
        }
        catch(error){
    ...
    static async placeOrder(order,clientObj){
        ...
        try{
            ...
            const values2 = [order_id, element.id, element.quantity, element.instructions];    
